				...
				default:
				{
					if ($s[4]=='r') {
						if (!empty($s[3])) {
							...
							echo "<div id=\"$keys\" class=\"div-input $keys\">$row[$keys] &nbsp;</div>\n";
						} 
					...
					{			
						...
						//echo "<input class=\"form-input\" type=\"text\" id=\"$s[3]\" value=\"$row[$val]\"> \n"; 
							if (!empty($s[10])) {
								if ($_COOKIE['recordset_action'] == 'insert') {
									$row[$val] = $_SESSION['user']['title'];
									$row[$keys] = $_SESSION['user']['id'];
								}
							}
							echo "<div id=\"$si[0]\" class=\"div-input $si[0]\">$row[$val] &nbsp;</div>\n";
							echo "<div class=\"div-select\">\n";
							...
							if ($dbhandle) {
								...
								if (!empty($MyTable)) {
									...
									AjaxShow(':::edit_related.php:related:'); 
									ShowElement('related-overlay'); 
									ShowElement('related')\">
									<img src=\"lib/img/recordset/table_row_insert.png\" class=\"menu\">
									</a> \n"; 
								}
								db::close($dbhandle);
							}
...
if (($_COOKIE['recordset_action'] == 'browse') || 
	($is_disabled_flag) ||  
...
<?php }?> 
<input type="button" value="<?php echo $tback;?>" OnClick="deletecookie('key'); deletecookie('tab'); deletecookie('recordset_action'); HideElement('forms'); HideElement('overlay'); window.location='main.php'">
