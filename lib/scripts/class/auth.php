...
if (!$_SESSION['db_admin']) {
	...
	if (!empty($dbhandle)) {
		...
		while ($rights_row = pg_fetch_assoc($result)) {
        	...
        	$filter_share = $rights_row['role_rights_share_records'];
        	...
        	$_SESSION['user_rights'][$table]['filter_share'] = $filter_share;
	        $_SESSION['user_rights'][$table]['accepts'] = $accepts;
    	    $_SESSION['user_rights'][$table]['accepts_column'] = $accepts_column;
		}
		$user_title = pg_fetch_assoc(pg_query("SELECT user_attribute_value
		FROM administration.users_attributes
		WHERE user_attribute_user_id = {$_SESSION['user']['id']}
		AND user_attribute_name = 'cn'"));
		$_SESSION['user']['title'] = $user_title['user_attribute_value'];
		db::close($dbhandle);
	}
  }
...
private function add_ad_user($user,$role) {
	...
	$passwd_row = pg_fetch_assoc(@pg_query("SELECT crypt('$password', gen_salt('MD5')) AS crypted"));
	$password = $passwd_row['crypted'];
	pg_query("INSERT INTO administration.users (user_role_id, user_login, user_passwd, user_active) VALUES ($role, '$user', '$password', DEFAULT)");
