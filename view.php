	...
	if (!empty($_COOKIE['current_table']))
	{
		...
		if (!empty($_COOKIE['recordset_action']))
		{
			...
			if ($rights === 't') {
				...
				echo "<div id=\"messages\" class=\"messages\" OnmouseOver=\"HideElement('related-overlay'); HideElement('related'); HideElement('messages'); HideElement('overlay'); HideElement('forms');\">Naudotojas „".$_SESSION['user']['user']."“ negali žiūrėti šios lentelės.</div>";
			}
		} else echo "<div id=\"errors\" class=\"errors\" OnmouseOver=\"HideElement('errors')\">Missing or invalid recordset_action cookie...</div>";
	}	else 
	{
		echo "<div id=\"messages\" class=\"messages\" OnmouseOver=\"HideElement('messages'); HideElement('overlay'); HideElement('forms')\">Pasirinkite lentelę...</div>";
	} 
?> 
